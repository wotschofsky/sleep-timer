...
el.display.circle.setAttribute(
  'stroke-dasharray',
  2 * el.display.circle.getAttribute('r') * Math.PI
);

const getTimeInSeconds = () =>
  Math.abs(parseFloat(el.inputs.hours.value || 0)) * 3600 +
...
const onInput = (event) => {
  if (event) {
    ...
    if (parsedValue < 0) {
      ...
      (event.target === el.inputs.seconds ||
        event.target === el.inputs.minutes) &&
      parsedValue > 59
    ) {
...
const startTimer = () => {
  ...
  el.display.circle.style.strokeDashoffset =
    2 * el.display.circle.getAttribute('r') * Math.PI;
